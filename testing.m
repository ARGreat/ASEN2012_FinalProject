...
idealLaunchEquations = [statevector(end,1),round(statevector(end,2),2),statevector(end,3),round(statevector(end,4),1)];
dist;
delta = 1;
%Get Fairly Close
while(abs(delta) > .1)
    dist = simulateLaunch(idealLaunchEquations, bounds,history);
    delta = 92-dist
    idealLaunchEquations = idealLaunchEquations +[0,0,.001,0]*delta/abs(delta);
end
%Get VERRRY CLOSE
while(abs(delta) > .01)
    dist = simulateLaunch(idealLaunchEquations, bounds,history);
    delta = 92-dist
    idealLaunchEquations = idealLaunchEquations +[0,0,.0001,0]*delta/abs(delta);
end

%Convert fractions to actual units
idealLaunchEquations = bounds(1,:)+idealLaunchEquations.*(bounds(2,:)-bounds(1,:));
